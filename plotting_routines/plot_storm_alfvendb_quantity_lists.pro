PRO PLOT_STORM_ALFVENDB_QUANTITY_LISTS,NEVRANGE=nEvRange, LOG_DBQUANTITY=log_DBQuantity, $
  ...
  FOR i=0,nStorms-1 DO BEGIN
     ...
     IF neg_and_pos_separ THEN BEGIN
        ...
        IF (tot_plot_i_pos_list[i])[0] GT -1 AND N_ELEMENTS(tot_plot_i_pos_list[i]) GT 1 AND ~(noPlots OR noMaxPlots) THEN BEGIN
        ...
        IF (tot_plot_i_neg_list[i])[0] GT -1 AND (N_ELEMENTS(tot_plot_i_neg_list[i]) GT 1) AND ~(noPlots OR noMaxPlots) THEN BEGIN
     ...
     ENDIF ELSE BEGIN
        IF tot_plot_i_list[i,0] GT -1 AND (N_ELEMENTS(tot_plot_i_list[i]) GT 1) AND ~(noPlots OR noMaxPlots)  THEN BEGIN
